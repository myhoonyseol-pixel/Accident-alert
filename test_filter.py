...
SAMPLES = [
    ...
    # 2026-08-31 실제 오탐 — 매체 이름이 사고 키워드로 오인된 사례.
    # 구글 뉴스의 '요약'이 「제목 + 매체명」이라 '경상매일신문'의 '경상'(가벼운 부상)이
    # 사고 키워드로 잡혔습니다. main.py에서 구글 뉴스 요약을 버리도록 고쳤습니다.
    # (제목만 보면 사고 키워드가 하나도 없어 정상적으로 걸러집니다)
    ("FALSE", "정한석 경북도의회 교육위원장, 칠곡 교육시설 공사현장 꼼꼼 점검"),
    ("FALSE", "군수, 신축 공사현장 안전관리 실태 꼼꼼 점검"),
    # 2026-09-01 실제 오탐 — '안전대'(추락방지 벨트)가 '안전대책'에 걸렸습니다.
    # 철도 기획기사인데 '철도 현장' + '안전대책' + '사망' 으로 통과했습니다.
    ("FALSE", "오봉역 참사 4년 만에 또 입환작업 사망…철도 현장 안전대책 어디까지 작동했나"),
    ("FALSE", "공사장 안전대진단 결과 발표…위험 현장 30곳 적발"),
    # '건설현장'과 '사망'이 진짜로 들어있어 키워드로는 못 막습니다. AI가 막습니다.
    ("AI",    "정부, 건설현장 추락 사망 줄이려 안전대책 발표"),

    # ── 2026-09-03 동사형 작업 표현 ─────────────────────────
    # 실제로 놓친 건: 2명 사망인데 '현장'도 '공사 중'도 없어 통째로 탈락했습니다.
    ("TRUE",  "아파트 6층 베란다 창틀 공사하던 2명 추락해 숨져"),
    ("TRUE",  "지붕 방수하던 60대 추락사"),
    ("TRUE",  "배관 설치하던 작업자 끼임 사고"),
    ("TRUE",  "옹벽 철거하다 무너져 2명 매몰"),
    ("TRUE",  "외벽 도색하는 인부 추락해 중상"),
    # '수리'는 일부러 뺐습니다 — 아래가 걸리기 때문입니다
    ("FALSE", "농기계 수리하던 70대 사망"),
    ("FALSE", "드라마 촬영하다 배우 부상"),

    # ── 2026-09-01 범위 확대 · 제조업 공장·물류창고 ──────────
    ("TRUE",  "평택 물류창고 화재…작업자 2명 부상"),
    ("TRUE",  "화성 반도체 공장 폭발…인명피해 확인 중"),
    ("TRUE",  "천안 페인트 원료 공장 화재…3명 사망·2명 실종"),
    ("TRUE",  "울산 석유화학단지 정유공장 폭발"),
    # 범위 밖 — 철도·운송·정비는 안 받기로 했습니다
    ("FALSE", "의왕역 입환 작업 중 코레일 직원 열차에 깔려 숨져"),
    ("FALSE", "트럭 정비하던 40대 근로자 끼임 사고로 숨져"),
    # 같은 유형의 함정들
